...
public class NeoUI : GameUI
{
    ...
    public void AnimateBar(RectTransform bar, Image image, float w)
    {
        StartCoroutine(BarAnim(bar, image, w));
    }

    private IEnumerator BarAnim(RectTransform bar, Image image, float w)
    {
        w = float.IsNaN(w) ? 0 : w;
        float t = 0;
        while (t < 1)
        {
            t += Time.deltaTime * 1f;
            SetBar(bar, image, Mathf.Min(t, 1) * w);
            yield return null;
        }
    }
