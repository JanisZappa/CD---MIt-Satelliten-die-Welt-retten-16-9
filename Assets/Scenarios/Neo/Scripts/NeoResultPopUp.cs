...
public class NeoResultPopUp : PopUp
{
    ...
    public override void ShowPopup(Action<int> callback = null)
    {
        ...
        for (int i = 0; i < 3; i++)
            neoui.AnimateBar(bars[i], barColors[i], NeoUI.BARS[i]);
        ...
        coins[0].sprite = coinSprites[NeoUI.PU];
        coins[1].sprite = coinSprites[NeoUI.PR];
        coins[2].sprite = coinSprites[NeoUI.SP];
    }
}
