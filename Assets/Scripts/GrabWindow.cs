...
public class GrabWindow : MonoBehaviour
{
    ...
    {
        if(beingDragged || PopUpManager.ShowingPanel || CFT_Marker.ShowingInfo)
            return false;
        
        offset =  pos - finger;
       
        beingDragged = InsideRect(pos, windowSizeA, finger) && !InsideRect(pos + new Vector2(0, yOffset), windowSizeB, finger);
    ...
    public void GrabUpdate(Vector2 finger)
    {
        ...
        return;
        DRAW.Rectangle(pos, new Vector2(windowSizeA.x * 2, windowSizeA.y * 2));
        DRAW.Rectangle(pos + new Vector2(0, yOffset), new Vector2(windowSizeB.x * 2, windowSizeB.y * 2));
    }
