...
public class BerryUI : GameUI
{
    ...
    public override void SetActive(bool show, SplitGame player1, SplitGame player2)
    {
        ...
        else
        {
            ...
            RatingUI.Scores[0] = new Ink.Score
            {
                maskHitRatio   = 0,
                wasteAreaRatio = 0
            };
            RatingUI.Scores[1] = new Ink.Score
            {
                maskHitRatio   = 0,
                wasteAreaRatio = 0
            };
            
            Debug.Log("Hiding Berry");
        }
    }
    ...
    private void LateUpdate()
    {
        if (player1 != null)
        {
            ...
            tablet.anchoredPosition = new Vector3(NewUI.res.x * wP.x / 16f, NewUI.res.y * wP.y / 9f);

            if (state < 2)
                RatingUI.Scores[state] = p1.GetScore;
        }
        ...
        bottomTools.localPosition = Vector3.up * (images.activeInHierarchy? SplitAnim.center * 200 - 200 : -200);
    }
}
